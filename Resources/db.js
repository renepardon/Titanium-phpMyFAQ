...
exports.selectDomains = function(_uri) {
	var retData = [];
	var db = Ti.Database.open(DATABASE_NAME);
	var rows = db.execute('SELECT `ROWID` AS `uid`, * FROM `domains` WHERE `uri`=?;', _uri);
	
	while (rows.isValidRow()) {
		retData.push({item:rows.fieldByName('name'), id:rows.fieldByName('uid')});
		rows.next();
	}
	db.close();
